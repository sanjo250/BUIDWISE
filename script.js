...
function makeMonthInputs() {
  $("monthsGrid").innerHTML = monthNames.map((month,i)=>`
    <div class="month-field"><label for="${monthIds[i]}">${month}</label>
    <input id="${monthIds[i]}" type="number" min="0" step="1" placeholder="kWh" aria-label="${month} electricity in kWh"></div>
  `).join("");
...
function createProfile(event) {
  ...
  if (!name || !type || ![area,occupancy,days,hours,annual].every(Number.isFinite) ||
      area<=0 || occupancy<=0 || days<=0 || days>365 || hours<=0 || hours>24 || annual<=0) {
    showMessage("Please complete every field with valid values.");
    return;
  }
  profile = {name,type,area,occupancy,days,hours,annual,epi:annual/area};
...
function fillExample() {
  const values = [8000,7500,8200,9000,10500,9800,9200,9400,8800,8500,7800,9000];
  monthIds.forEach((id,i)=>$(id).value=values[i]);
  drawMonthlyChart();
}
function drawMonthlyChart() {
  const values = monthIds.map(id => $(id) && $(id).value.trim() !== "" ? Number($(id).value) : null);
  const valid = values.filter(v => v !== null && Number.isFinite(v) && v >= 0);
  if (!valid.length) {
    $("monthChart").innerHTML = '<p class="empty-chart">Add monthly values or fill the illustrative example to see the chart.</p>';
    return;
  }
  const max = Math.max(...valid,1);
  $("monthChart").innerHTML = values.map((v,i) => {
    const usable = v !== null && Number.isFinite(v) && v >= 0;
    const height = usable ? Math.max(2,(v/max)*100) : 0;
    return `<div class="bar-column" title="${monthNames[i]}: ${usable ? number(v)+" kWh" : "not entered"}">
      <div class="bar" style="height:${height}%"></div><span>${monthNames[i]}</span></div>`;
  }).join("");
}
function runScenario() {
  ...
  if (![reduction,cost,tariff,factor].every(Number.isFinite) || reduction<=0 || cost<0 || tariff<0 || factor<0) {
  ...
  lastSimulation = {type,reduction:reduction*100,cost,tariff,factor,saved,scenarioEnergy,billSaving,carbon,payback};
  ...
  $("payback").textContent = payback === null ? "Not available" : (payback===0 ? "0 years" : `${number(payback,1)} years`);
  $("recommendation").textContent = billSaving<=0
    ? "With a zero tariff or no energy savings, simple payback cannot be meaningfully estimated."
    : payback<=3 ? "Under these assumptions, the simple payback is within 3 years. Check costs and savings against real data."
    : payback<=8 ? "Under these assumptions, payback is within 8 years. Compare feasibility and the reliability of the inputs."
...
function exportSummary() {
  ...
  lines.push("", "LIMITATION: Outputs are illustrative estimates based on user-entered assumptions. They are not measured savings or a professional energy audit.");
...
$("reduction").addEventListener("input", () => $("reductionValue").textContent=$("reduction").value);
